...
本报告于 2026-09-18 按修订后的统一来源审计标准复核：UTS、USYD、UNSW 使用同一澳大利亚框架；没有学校专项三维再分发授权不自动阻止自行制作的几何公开。对象 provenance 记录共 1,808 条，其中 714 条缺少 `source` 字段；这是历史元数据缺口，不是发现 714 个第三方模型。具体受限资产仍按对象级别暂缓或移除。
